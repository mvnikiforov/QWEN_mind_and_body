...
export function SectionHead({
  ...
  return (
    <div className="max-w-3xl">
      ...
      <Reveal delay={100}>
        <h2 className={`mt-4 sm:mt-5 font-display font-medium text-[clamp(28px,4.4vw,52px)] leading-[1.06] tracking-[-0.01em] ${t}`}>
          {title}
        </h2>
      </Reveal>
      {sub && (
        <Reveal delay={190}>
          <p className={`mt-4 sm:mt-5 max-w-2xl text-[14.5px] sm:text-base leading-relaxed ${s}`}>{sub}</p>
        </Reveal>
      )}
    </div>
  );
