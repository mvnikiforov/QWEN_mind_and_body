      ...
      <div className="grid gap-8 sm:gap-10 lg:grid-cols-12">
        ...
        <div className="lg:col-span-4">
          <div className="lg:sticky lg:top-28">
            ...
            {/* Мобильные пилюли — горизонтальный скролл */}
        ...
        <div className="lg:col-span-8">
          <div key={group.id} className="relative">
            <span aria-hidden className="pointer-events-none absolute -top-10 right-0 select-none font-display text-[100px] italic leading-none text-ink/6 sm:-top-14 sm:text-[150px] md:text-[190px]">
              „
            </span>
            <div className="relative grid gap-4 sm:gap-5 sm:grid-cols-2">
              {group.reviews.map((r, i) => (
                <div
                  ...
                  className="fadeup flex flex-col rounded-[22px] sm:rounded-[26px] border border-line bg-card/75 p-5 backdrop-blur-sm transition-all duration-500 hover:-translate-y-1.5 hover:border-gold/60 hover:bg-card hover:shadow-[0_32px_64px_-36px_rgba(35,33,29,0.5)] sm:p-7"
                  style={{ animationDelay: `${i * 110}ms` }}
                >
                  <div className="flex items-baseline justify-between gap-2 sm:gap-3">
                    <p className="font-display text-[18px] sm:text-[21px] font-semibold leading-tight text-gold-deep">{group.label}</p>
                    <span className="shrink-0 rounded-full bg-ink/6 px-2.5 py-1 sm:px-3 sm:py-1.5 text-[9px] sm:text-[10px] font-extrabold uppercase tracking-[0.14em] text-ink-soft">
                      {group.note}
                    </span>
                  </div>
                  <p className="mt-3 sm:mt-4 grow text-[13.5px] sm:text-[14px] leading-[1.7] sm:leading-[1.75] text-ink">«{r.text}»</p>
                  <div className="mt-4 sm:mt-6 flex items-center gap-3 border-t border-dashed border-line pt-4 sm:pt-5">
                    ...
                    <span className="leading-tight">
                      <span className="block text-[13px] sm:text-[14px] font-extrabold">{r.name}</span>
                      <span className="block text-[10.5px] sm:text-[11.5px] font-semibold text-ink-faint">клиент Валерии</span>
                    </span>
                    <svg className="ml-auto h-4 w-4 sm:h-5 sm:w-5 text-gold/70" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
            ...
            <p className="mt-4 sm:mt-6 text-[10.5px] sm:text-[11.5px] font-medium text-ink-faint">
