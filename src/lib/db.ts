/* ============================================================
   ...
   orders, events, posts, content, session). Версия схемы: 4.
   ============================================================ */
...
export const IMG = {
  ...
  stones: "https://image.qwenlm.ai/generated-images/dd2aa49a-b50c-496c-932e-e7865d370aa8/_result.png",
};
  ...
  return {
    ...
    services: [
      {
        ...
        variants: [
          ...
          },
          {
            ...
            price: 2500,
            priceUnit: "разовая встреча",
            packLabel: "Абонемент: 4 встречи — 8 000 ₽ (2 000 ₽/встреча)",
            packBenefit: "−2 000 ₽",
            description:
              "Длительная групповая работа для глубоких изменений. Пространство поддержки и обратной связи в терапевтической мини-группе.",
          },
        ],
      },
      {
      ...
      },
      {
        id: "balance",
        title: "Пакет «Баланс»",
        subtitle: "Терапия + практики · единый комплекс",
        variants: [
          {
            id: "ind",
            mode: "individual",
            image: IMG.stones,
            duration: "4 сессии по 50 мин + 4 практики по 2 часа",
            price: 25600,
            priceUnit: "весь пакет",
            packLabel: "4 терапевтические сессии + 4 индивидуальные практики — обычно 32 000 ₽",
            packBenefit: "−6 400 ₽",
            badge: "Выгода от 20%",
            description:
              "Сессии и телесные практики в едином ритме: разум и тело работают вместе, поэтому изменения закрепляются глубже и устойчивее.",
          },
          {
            id: "grp",
            mode: "group",
            image: IMG.stones,
            duration: "4 встречи мини-группы + 4 групповые практики",
            price: 14000,
            priceUnit: "весь пакет",
            packLabel: "4 встречи мини-группы (по 4 ч) + 4 практики (1,5–2 ч) — обычно 18 000 ₽",
            packBenefit: "−4 000 ₽",
            badge: "Выгода от 20%",
            description:
              "Групповая терапия и практики в одном абонементе: поддержка круга плюс регулярная телесная разгрузка.",
          },
        ],
      },
    ],
    ...
    events: [
      ...
      },
      {
        ...
        actions: [
          ...
          { label: "Купить абонемент", kind: "book" },
        ],
      },
      {
        id: "ev-mind",
        title: "Mindfulness-практика",
        when: "Еженедельно по субботам",
        time: "10:00",
        format: "online",
        price: "600 ₽ разовое",
        priceNote: "курсом — выгоднее и полезнее",
    ...
    posts: [
      ...
      },
      {
        ...
        text: "Сегодня в студии было особенно тепло. После шавасаны меняются лица — уходят зажимы, и человек словно возвращается к себе.",
        image: IMG.studio,
      },
      {
        id: "p-3",
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && parsed.version === 4) {
        /* аддитивные миграции для существующих пользователей */
        if (!parsed.services.some((s) => s.id === "balance")) {
          parsed.services.push(seed().services[2]);
        }
        if (parsed.content.culture.title !== "Мультикультурный подход" && parsed.content.culture.title === "Бережность к культурному коду") {
          parsed.content.culture.title = "Мультикультурный подход";
        }
        return parsed;
      }
    }
...
export function createOrder(payload: Omit<Order, "id" | "createdAt" | "status">) {
  const order: Order = { ...payload, id: uid(), createdAt: new Date().toISOString(), status: "new" };
  mutate((db) => {
